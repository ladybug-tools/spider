	...
	function initHeadsUp() {
		...
		if ( !divHeadsUp ) {
			...
			divHeadsUp.style.cssText =
				'background-color: #f8f8f8; border-radius: 8px; display: block; left: calc( 100% - 400px ); '+
...
//			window.addEventListener( 'mouseup', onMouseUpDraggable, false );
		...
		if ( butHeadsUp.style.backgroundColor !== 'var( --but-bg-color )' ) {
			...
			renderer.domElement.addEventListener( 'click', HUD.onRendererMouseMoveHUD, false );
			renderer.domElement.addEventListener( 'touchstart', HUD.onRendererTouchStartHUD, false );
...
// needs big clean up
	...
	HUD.setHeadsUp = ( event ) => {
			...
			if ( Array.isArray( adjacenciesTxt ) === true ) {
				...
				if ( space1 && space2 ) {
					...
					adjacenciesTxt =
						...
						'<hr>' +
						'adjacenct 1:  <button onclick=HUD.toggleSpace("' + space1.id + '"); >' + space1.id + '</button>' + b +
						...
						'<hr>' +
						'adjacenct 2: <button onclick=HUD.toggleSpace("' + space2.id + '"); >' + space2.id + '</button>' + b +
		...
		//						( space2.buildingStoreyIdRef ? 'buildingStoreyIdRef: ' + space2.buildingStoreyIdRef + b : '' )  +
				...
				adjacenciesTxt =
					'<hr>' +
					'adjacenct space id: <button onclick=HUD.toggleSpace("' + space.id + '"); >' + space.id + '</button>' + b +
		...
		txt =
			'<p>' +
			'id: <button onclick=HUD.toggleSurface("' + data.id + '")  >' + data.id + '</button>' + b +
...
//		if ( selSurface.selectedIndex < 0 ) { alert( 'Please first select a surface and make an edit' ); return; }
	...
	HUD.onRendererMouseDownHUD = ( event ) => {
	...
	HUD.onRendererTouchStartHUD = ( event ) => {
	...
	function toggleHeadsUpOff() {
		...
		renderer.domElement.removeEventListener( 'click', HUD.onRendererMouseMoveHUD, false );
		renderer.domElement.removeEventListener( 'click', HUD.onRendererMouseDownHUD, false );
