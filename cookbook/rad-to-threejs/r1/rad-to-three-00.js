...
function setThreeJsWindowUpdate() {
	...
	//console.log( 'arrRadJSon', arrRadJSon );
...
function drawPolygon ( polygon ) {
	...
	*/
	...
	//points = polygon.vertices.map( item => v( item[ 0 ], item[ 1 ], item[ 2 ] ) );

	points = polygon.vertices.map( item => new THREE.Vector3().fromArray( item ) );
...
*/
	...
	if ( points.length === 0 ) {
		...
		material = new THREE.MeshNormalMaterial( {  opacity: rad.opacity, side: 2, transparent: true } );
