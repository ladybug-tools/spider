...
* Loads several sample charts
* User selected variables applied to XYZ axes, size, shape and color
	* Variable names, minimum values and maximums value are displayed for each axis
* Mouseover any data point tO
	* View its associated image in the menu
	* View its data in a heads-up display adjacent to the pointer
* Filters generated by algorithm for variables that are lists of text items
* Nurmeric filters under construction
