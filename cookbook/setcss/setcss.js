...
function setTheme() {
	...
	const txt1 = themesBootswatch.map( theme => {
		...
		return `<button class="btn btn-sm" onclick=updateCss("${ link }"); style="${ theme[name] }" >${ bingo }${ name }${ bingo }</button> `;
	...
	for ( let theme of themesOthers ) {
		...
		txt += `<button class="btn btn-sm" onclick=updateCss("${ theme.link }"); style="${ theme.name }" >${ theme.name }</button> `;
