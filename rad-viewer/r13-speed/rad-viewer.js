/* Copyright 2018 Ladybug Tools authors. MIT License */
	...
	rad.setVertices = function() {
		...
		for ( let i = 1; i < rad.items.length; i++ ) {
			...
			if ( rad.items[ i ] === '' ||  rad.items[ i ].includes( 'opening')  ) {
				...
				const items = rad.items[ i ].trim().replace( /\t/g, ' ' ).replace( / {2,}/g, ' ' ).split( ' ' );
				//console.log( 'items', items );

				const material = items[ 2 ];
				//console.log( 'material', material );

				//console.log( 'rad.data[ i ][ 2 ]', rad.data[ i ][ 2 ] );

				if ( rad.data[ i ][ 2 ] ) {

				const arr = rad.data[ i ][ 2 ].trim().replace( /\t/g, ' ' ).replace( / {2,}/g, ' ' ).split( ' ' ).map( item => parseFloat( item ) );
		...
		rad.geomMaterial = new THREE.MeshPhongMaterial( { color: 'red', opacity: rad.opacity, side: 2, transparent: true } );
		...
		rad.zoomObjectBoundingSphere( rad.edges );
	...
	rad.drawPolygon = function( i ) {
		...
		if ( points.length === 0 ) {
			...
			//rad.geomMaterial = new THREE.MeshBasicMaterial( { color: color, opacity: rad.opacity, side: 2, transparent: true } );

			//scene.add( rad.meshes ); //, rad.edges );

			//zoomObjectBoundingSphere( rad.meshes );

			//const edgesGeometry = new THREE.EdgesGeometry( rad.geom );
	...
	rad.zoomObjectBoundingSphere = function( obj ) {
		...
		lightDirectional.position.copy( center.clone().add( new THREE.Vector3( -0.6 * radius, -0.6 * radius, 0.6 * radius ) ) );
		lightDirectional.shadow.camera.scale.set( 0.08 * radius, 0.08 * radius, 0.004 * radius );
