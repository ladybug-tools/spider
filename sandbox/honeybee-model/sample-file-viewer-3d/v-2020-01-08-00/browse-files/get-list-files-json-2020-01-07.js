...
GLF.getMenu = function() {
	...
	<details ontoggle=GLF.getFileNames(); open>
		...
		<select id=GLFselFiles onchange=GLF.getFileJson(this.value) size=10 style=overflow:auto;width:100%; ></select>
	...
	<details open>
		...
		<p id=GLFpJsonTreeViewHelp onmouseout=GLFpJsonTreeViewHelp.hidden=true hidden >
		JSON rendered to a tree view using <a href="https://github.com/pgrabovets/json-view" target="_blank">json-view</a>
		<br>
		</p>
...
GLF.getFileJson = function ( index ) {
	...
	xhr.onload = ( xhr ) => {
		...
		GLFdivFileLoaded.innerHTML =
			`<p style=overflow:auto;width:100%; >File Loaded: <a href="${ GLF.source + GLF.filesData[ index ].path.split( "/" ).pop() }" target="_blank">${ GLF.filesData[ index ].path }</a></p>`;
