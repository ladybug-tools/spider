/* Copyright 2018 Ladybug Tools authors. MIT License */
	...
	rad.drawPolygon = function( i ) {
		...
		*/
		...
		if ( points.length === 0 ) {
			...
			//shape.position.multiplyScalar(1.2);
			...
			if (points.length == 4) {
				rad.edges.add( surfaceEdge );
				surfaceEdge.renderOrder = -999;
				surfaceEdge.position.z += 0.01;
				surfaceEdge.scale.set(1.001, 1.001,1);
				//surfaceEdge.onBeforeRender = function( renderer ) { renderer.clearDepth(); };
	...
	rad.drawShape = function( vertices, material ) {
		...
		// this will create only one triangle to use
		// it's normal, no mather how many vertices
		const plane = rad.getPlane( vertices );
  		
  		// copy the rotation of the triangle
		obj.lookAt( plane.normal );
		obj.quaternion.conjugate();
		obj.updateMatrixWorld();

		// apply obj matrix to each vertex
