...
//			'<?xml version="1.0" encoding="UTF-16"?>\n' +
			...
			'\t\t<Building id="addNodebuilding-' + campus.buildingId + '" buildingType="Office" >\n' +
	...
	function getBuildingStoreys( campus ) {
		...
		for ( var i = 0; i < campus.storeys; i++ ) {
			...
			textBuildingStoreys +=
				'\t\t\t<BuildingStorey id="addNodestorey-' + storey + '" >\n' +
...
console.log( 'path', pathInterior );
		...
		for ( var j = 0; j < spaces.length; j++ ) {
			...
			textSpace +=
				'\t\t\t<Space ' +
					'id="addNodespace-' + space.id + '" ' +
					'buildingStoreyIdRef="addNodestorey-' + space.storey + '" ' +
					'zoneIdRef="addNodezone-' + space.id + '" ' +
...
//		const material = new THREE.MeshNormalMaterial( { opacity: 0.85, transparent: true } );
		...
		let textSingleSurfaces =
			...
			'\t\t<Surface surfaceType="' + quad.type + '" id="addNodesurface-' + quad.surfaceId + '" >\n' +
				'\t\t\t<Name>' + quad.name + '</Name>\n' +
			'\t\t\t<CADOjectId>none</CADOjectId>\n' +
			'\t\t\t<AdjacentSpaceId spaceIdRef="addNodespace-' + quad.spaceId1  + '" />\n' +
			( quad.spaceId2 ? '\t\t\t<AdjacentSpaceId spaceIdRef="addNodespace-' + quad.spaceId2  + '" />\n' : '' ) +
			'\t\t\t<PlanarGeometry>\n' +
			'\t\t\t\t<PolyLoop>\n' +
		'';
...
//		let mesh;
		...
		textOpening +=
			'\t\t\t<Opening openingType="FixedWindow" id="addNodeopening-' + openingId + '" >\n' +
			'\t\t\t\t<PlanarGeometry>\n' +
			'\t\t\t\t\t<PolyLoop>\n' +
		'';
		openingId ++;
...
//		let overhangId = ( storey.userData.storey - 1 ) * path.length + 1;
		...
		for ( let i = 0; i < storey.children.length; i++ ) {
			...
			textOver +=
				'\t\t<Surface surfaceType="Shade" id="addNodeshade-' + ( overhangId ++ ) + '" >\n' +
...
//		const material = new THREE.MeshNormalMaterial( { opacity: 0.85, transparent: true } );
		...
		for ( let i = 0; i < faces.length; i++ ) {
			...
			textSurfaces +=
				'\t\t<Surface surfaceType="Shade" id="addNodeshade-' + ( i + 1 ) + '" >\n' +
